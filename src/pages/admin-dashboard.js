  ...
  <script>
    ...
    async function loadDashboardStats() {
      try {
        ...
        if (response.success) {
          const { totalProducts, totalInquiries, pendingInquiries, recent_inquiries } = response.data;

          document.getElementById('stat-products').textContent = totalProducts || 0;
          document.getElementById('stat-inquiries').textContent = totalInquiries || 0;
          document.getElementById('stat-pending').textContent = pendingInquiries || 0;
    ...
    async function loadInquiries() {
      try {
        ...
        if (response.success) {
          ...
          console.log('[Inquiries] Total received:', inquiries.length, inquiries);
